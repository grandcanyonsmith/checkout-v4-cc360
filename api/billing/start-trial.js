...
  try {
    // Read required parameters from the request body
    const { customerId, paymentMethodId, priceId, userInfo, affiliateId, referrer, campaign } = req.body;
    
    // CONSOLIDATE AFFILIATE ID: Use ID sent in the main body, or look in userInfo (fallback)
    const finalAffiliateId = affiliateId || userInfo.affiliateId || 'none';
...
    });
    
    // Create a consolidated metadata object for user info and tracking
    const userMetadata = {
        firstName: userInfo.firstName,
        lastName: userInfo.lastName,
        email: userInfo.email,
        phone: userInfo.phone,
        affiliateId: finalAffiliateId, // Use the determined ID
...
      // CRITICAL FIX: Ensure metadata key is 'affiliateId' (camelCase)
      metadata: {
        signup_source: 'course_creator_360_trial',
        user_info: JSON.stringify(userMetadata), 
        affiliateId: userMetadata.affiliateId, // FIX APPLIED HERE
